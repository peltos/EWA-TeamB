<div class="container streamers-container">
  ...
  <div class="tabBar-container">
    <div class="tabBar-inner">
      <div class="tab">
      <button class="tab--item" onclick="openCity(event, 'all')">All</button>
      <button class="tab--item" onclick="openCity(event, 'fifa18')">FIFA 18</button>
      <button class="tab--item" onclick="openCity(event, 'battlefield4')">Battlefield 4</button>
      <button class="tab--item" onclick="openCity(event, 'fortnite')">Fortnite</button>
      </div>
    </div>
  </div>
    ...
    if (!empty($streamers)) {
        ...
        foreach ($streamers as $key => $item) {
           if($item["type"]["name"] == "Battlefield 4"){
              echo '<div class="stream-tile"><a class="streamers--item" href="https://mixer.com/'.$item["token"].'" target="_blank">';
              echo '<div class="streamers--item__container">';
              echo '<img src="' . $item["type"]["coverUrl"] . '"/> <br>';
              echo '<span class="streamer-title">' . $item["token"] . ' - ' . $item["name"] . ' </span>';
              echo '</br>';
